...
class WeatherSkill(MycroftSkill):
    ...
    def initialize(self):
        ...
        self.add_event('mycroft-weather.mycroftai.setting', self.test_setting_event)
        self.add_event(msg_type, self.handle_idle)
        self.add_event('mycroft.mark2.collect_idle',
                       self.handle_collect_request)
        #self.gui.show_config('Weather Units', "select", self.settings.get("units", ""), ["default", "celcius", "fahrenheit"])
       
        self.schedule_for_daily_use()

        # self.test_screen()    # DEBUG:  Used during screen testing/debugging
        
    def test_setting_event(self, message):
        self.gui.update_config(message.data["setting_key"], message.data["setting_value"])
        self.settings["units"] = message.data["setting_value"]
        self.settings.store()
